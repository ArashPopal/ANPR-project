...
class Guest_Registeration(Tk):
    def __init__(self, *args, **kwargs):
        ...
        FName1.grid(row=1, column=3,padx=5)
        # _____________________________________NAME______________________________________________#
    ...
    def add_button_click1(self):
        ...
        if data is not None:
            self.cur.execute("insert into Guests(GNational_ID,Name,FName,LName,GLPlate,ReasonToVisite,HName,HFName,"
                             "HLName,HouseNo) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                             (self.GNational_ID.get(),self.Name.get(), self.FNAME.get(), self.LName.get(),
                              self.GLPlate.get(), self.Reason_to_visit.get(), self.HName.get(), self.HFName.get(),
                              self.HLName.get(),
                              self.HouseNo.get()))
